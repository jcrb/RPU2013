...
\section{Exhaustivité des qualitative}
...
%on fabrique un objet **a** qui fait la somme pae date des passages aux urgences:
...
L'ensemble des SU ont déclaré \np{193102} passages en 2013, 
soit une moyenne de \np{916} passages par jour (extrèmes 2 et \np{1160})
...
\begin{kframe}
\begin{alltt}
s <- \hlfunctioncall{summary}(d1$MODE_SORTIE)
na <- s[6]
t <- \hlfunctioncall{table}(d1$MODE_SORTIE)
r <- \hlfunctioncall{round}(\hlfunctioncall{prop.table}(t) * 100, 2)
c <- \hlfunctioncall{cbind}(t, r)
\hlfunctioncall{colnames}(c) <- \hlfunctioncall{c}(\hlstring{"n"}, \hlstring{"%"})
\hlfunctioncall{xtable}(c, caption = \hlstring{"Mode de sortie des urgences"}, label = \hlstring{"tab.sortie"}, align = \hlstring{"|l|r|r|"}, 
    digits = \hlfunctioncall{c}(0, 0, 2), table.placement = \hlstring{"ht"}, caption.placement = \hlstring{"top"})
\end{alltt}
\end{kframe}% latex table generated in R 2.15.1 by xtable 1.7-1 package
% Wed Aug  7 19:57:24 2013
\begin{table}[ht]
\centering
\begin{tabular}{|l|r|r|}
  \hline
 & n & \% \\ 
  \hline
NA & 0 & 0.00 \\ 
  Mutation & 38183 & 23.27 \\ 
  Transfert & 2762 & 1.68 \\ 
  Domicile & 123148 & 75.05 \\ 
  Décès & 2 & 0.00 \\ 
   \hline
\end{tabular}
\caption{Mode de sortie des urgences} 
\label{tab.sortie}
\end{table}



Le mode de sortie est affiné par la rubrique DESTINATION avec la ventilation suivante:
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlfunctioncall{summary}(d1$DESTINATION)
\end{alltt}
\begin{verbatim}
##     NA    MCO    SSR    SLD    PSY    HAD    HMS   NA's 
##      0  40127     33      8    703      1     18 152212
\end{verbatim}
\begin{alltt}

\hlfunctioncall{table}(d1$DESTINATION)
\end{alltt}
\begin{verbatim}
## 
##    NA   MCO   SSR   SLD   PSY   HAD   HMS 
##     0 40127    33     8   703     1    18
\end{verbatim}
\begin{alltt}

\hlfunctioncall{table}(d1$DESTINATION, d1$GRAVITE)
\end{alltt}
\begin{verbatim}
##      
##           1     2     3     4     5     D     P
