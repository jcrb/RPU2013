\documentclass[12pt,english,french,twoside]{report}\usepackage{graphicx, color}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcomment{# drop.levels permet d'éliminer le level 0 qui est nul}
a <- \hlfunctioncall{drop.levels}(d1$ORIENTATION)
\hlfunctioncall{summary}(a)
\end{alltt}
\begin{verbatim}
...
\hlfunctioncall{table}(a, useNA = \hlstring{"always"})
\end{alltt}
\begin{verbatim}
## a
##   CHIR  FUGUE    HDT     HO    MED   OBST    PSA    REA    REO     SC 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
a <- d1[d1$MODE_SORTIE == \hlstring{"Domicile"}, ]
\hlfunctioncall{summary}(\hlfunctioncall{as.factor}(a$ORIENTATION))
\end{alltt}
\begin{verbatim}
##   CHIR  FUGUE    HDT     HO    MED   OBST    PSA    REA    REO     SC 
...
\end{verbatim}
\begin{alltt}
t <- \hlfunctioncall{table}(\hlfunctioncall{as.factor}(a$ORIENTATION))
\hlfunctioncall{round}(\hlfunctioncall{prop.table}(t) * 100, 2)
\end{alltt}
\begin{verbatim}
## 
...
\end{verbatim}
\begin{alltt}
\hlfunctioncall{tab1}(\hlfunctioncall{as.factor}(a$ORIENTATION), sort.group = \hlstring{"decreasing"}, horiz = TRUE, cex.names = 0.8, 
    xlab = \hlstring{""}, main = \hlstring{"Orientation des patients non hospitalisés"}, missing = F)
\end{alltt}
\end{kframe}
