...
a[a=="670000165"]<-"Sav"
unique(a)
d09$FINESS<-as.factor(a)
rm(a)
```
Transformation en facteur:
```{r}
d09$CODE_POSTAL<-as.factor(d09$CODE_POSTAL)
d09$COMMUNE<-as.factor(d09$COMMUNE)
d09$SEXE<-as.factor(d09$SEXE)
d09$TRANSPORT<-as.factor(d09$TRANSPORT)
d09$TRANSPORT_PEC<-as.factor(d09$TRANSPORT_PEC)
d09$GRAVITE<-as.factor(d09$GRAVITE)
d09$ORIENTATION<-as.factor(d09$ORIENTATION)
d09$MODE_ENTREE<-factor(d09$MODE_ENTREE,levels=c(0,6,7,8),labels=c('NA','Mutation','Transfert','Domicile'))
d09$PROVENANCE<-factor(d09$PROVENANCE,levels=c(0,1,2,3,4,5,8),labels=c('NA','MCO','SSR','SLD','PSY','PEA','PEO'))
d09$MODE_SORTIE<-factor(d09$MODE_SORTIE,levels=c(0,6,7,8,4),labels=c('NA','Mutation','Transfert','Domicile','Décès'))
d09$DESTINATION<-factor(d09$DESTINATION,levels=c(0,1,2,3,4,6,7),labels=c('NA','MCO','SSR','SLD','PSY','HAD','HMS'))
```
Création d'une variable AGE:
```{r-age1}
d09$AGE<-floor(as.numeric(as.Date(d09$ENTREE)-as.Date(d09$NAISSANCE))/365)
```
Correction des ages supérieurs à 120 ans (3 cas) ou inférieur à 0 (2 cas)
```{r age}
d09$AGE[d09$AGE > 120]<-NA
d09$AGE[d09$AGE < 0]<-NA
```
etape 3: sauvegarde des données nettoyées
-----------------------------------------
```{r-sauvegarde}
# write.table(d07,"rpu2013_07.txt",sep=',',quote=TRUE,na="NA")
save(d09,file="rpu2013d09.Rda")
# load("rpu2013d09.Rda")
```
...
-----------------------------
*d0108* est la base annuelle provisoire
```{r}
load("rpu2013d0108.Rda")
d0109<-rbind(d0108,d09)
save(d0109,file="rpu2013d0109.Rda")
```
